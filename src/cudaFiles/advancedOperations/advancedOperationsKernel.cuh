...
template <typename T>
__global__ void transpose(const T*  __restrict__ a, T* b, int ROWS, int COLUMNS) {
...
template <typename T>
__global__ void dot(const T*  __restrict__ a, const T* __restrict__  b, T* c, int ROWS, int COLUMNS) {
    /*
    ...
    */
    ...
    if (tidX < COLUMNS && tidY < ROWS){
        ...
        for (unsigned int i = 0; i < COLUMNS; i ++){
            intermediateValue += *(a+tidY*ROWS+i) * *(b+i*COLUMNS+tidY) ;
            }
